...
class Layer:
    def __init__(self, layer_index, layer_details, input_width):
        self.input_index = 0
        ...
        for p in range(self.number_of_replicates):
            self.pipelines.append(pipeline.Pipeline(input_width))
    ...
    def run(self, layers, output_buffer, overlapped):
        if(self.layer_index < len(layers) - 1 and self.input_index != layers[self.layer_index + 1].input_index):
            return 0
