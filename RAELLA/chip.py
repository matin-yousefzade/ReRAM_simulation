...
CYCLE_TIME = 100        # ns
CHIP_AREA = 85.324      # mm2
PEAK_COMPUTATIONAL_DENSITY = {8 : 9421.830, 16 : 2355.457}   # GOP/s/mm2
...
ENERGY_POWER['quantization_buffer'] = 84.228            # fJ
ENERGY_POWER['IR'] = 60.546                             # fJ
ENERGY_POWER['COU'] = 0.424                             # fJ
ENERGY_POWER['DAC'] = 390.625                           # fJ
ENERGY_POWER['CB'] = 480000                             # fJ
ENERGY_POWER['psubbuf'] = 2.3                           # fJ
ENERGY_POWER['SH'] = 0.976                              # fJ
ENERGY_POWER['ADC'] = 1420                              # fJ
ENERGY_POWER['SA'] = 156.25                             # fJ
ENERGY_POWER['OR'] = 5.615                              # fJ
...
class Chip:
    ...
    def map_weights(self, network_address, number_of_chips, bit_width):
        ...
        for l in range(len(network_details)):
            if(network_details.loc[l, 'type'] != 'pool'):
                ...
                number_of_cols = math.ceil(network_details.loc[l, 'M'] / (self.cb_size // (3 * bit_width / 8)))
    ...
    def run(self, network_address, number_of_chips, bit_width, overlapped, pipelined, plot):
        ...
        output_per_cb = self.cb_size // (3 * bit_width / 8)
        energy = {}
        ...
        energy['quantization_buffer'] = 2 * (total_cb_cycles // bit_width) * output_per_cb * 32 * ENERGY_POWER['quantization_buffer']
        energy['IR'] = 2 * total_cb_cycles * self.cb_size * ENERGY_POWER['IR']
        energy['COU'] = (total_cb_cycles // bit_width) * output_per_cb * ENERGY_POWER['COU']
        energy['DAC'] = total_cb_cycles * self.cb_size * ENERGY_POWER['DAC']
        energy['CB'] = total_cb_cycles * ENERGY_POWER['CB']
        energy['psubbuf'] = total_cb_cycles * self.cb_size * ENERGY_POWER['psubbuf']
        energy['SH'] = total_cb_cycles * self.cb_size * ENERGY_POWER['SH']
        energy['ADC'] = total_cb_cycles * self.cb_size * ENERGY_POWER['ADC']
        energy['SA'] = total_cb_cycles * output_per_cb * ENERGY_POWER['SA']
        energy['OR'] = 2 * total_cb_cycles * output_per_cb * 16 * ENERGY_POWER['OR']
    ...
    def simulate(self, file_address, benchmarks, number_of_chips, bit_width, overlapped, pipelined, plot):
        ...
        metrics = ['latency', 'cycle_per_inference', 'utilization', 'throughput', 'computational_density', 'cd_error', 'energy', 'energy_efficiency', 'minimum_required_chips', 'hyper_transport', 'router', 'eDRAM', 'eDRAM_bus', 'quantization_buffer', 'IR', 'COU', 'DAC', 'CB', 'psubbuf', 'SH', 'ADC', 'SA', 'OR']
